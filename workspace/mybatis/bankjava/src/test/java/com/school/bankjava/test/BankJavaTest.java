...
//	}
	...
	public void 계정상세_foreach(){
		...
		assertTrue((beforeTotalCount + 10) == afterTotalCount);
	}
	
	@Test
	public void 사용자ResultMap(){
		try(SqlSession sqlSession = sqlSessionFactory.openSession()){
			dao = new BankJavaDao(sqlSession);
			usersVo = dao.selectUsersVoResultMap();
		}
		assertTrue(usersVo != null);
		assertTrue(usersVo.getAccountsList().size() > 0);
	}
